...
1) EPITAPHS: Find the newest reviews/foundry/round-NNN.json. If some reports/gauntlet-*.json covers all its idea names AND any name lacks an entry in reviews/foundry/dead-ideas.json, add one entry per missing idea: name; lineage 'foundry rNNN · <lens>'; tested (date + window); result (n, pnl, edge/slot, win rate); verdict 'dead' (or 'holdout_survivor' ONLY if it beat its own kill_criterion — never 'live_arm'); an epitaph with the mechanism-level lesson; and a gradient object {predicted_fires_per_sym_day, actual, net_expectancy_pct}. Promote patterns seen twice into failure_lessons; maintain total_ideas_tested. Validate JSON parses, commit, push.
