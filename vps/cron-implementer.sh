...
1) EPITAPHS: Find the newest reviews/foundry/round-NNN.json. If some reports/gauntlet-*.json covers all its idea names AND any name lacks an entry in reviews/foundry/dead-ideas.json, add one entry per missing idea: name; lineage 'foundry rNNN · <lens>'; tested (date + window); result (n, pnl, edge/slot, win rate); verdict 'dead' (or 'holdout_survivor' ONLY if it beat its own kill_criterion — never 'live_arm'); an epitaph with the mechanism-level lesson; and a gradient object {predicted_fires_per_sym_day, actual, net_expectancy_pct}. If reviews/foundry/premortem-NNN.md exists for this round, cite its per-idea verdict in each epitaph and add premortem_correct: true/false to the gradient object — the premortem is a forecaster being scored. Promote patterns seen twice into failure_lessons; maintain total_ideas_tested. Then refresh STATUS.md: bump its snapshot date and update the 'Current research state' numbers (rounds complete, ideas tested, lessons count, and the one-lead paragraph if the newest verdicts changed the picture) — keep its voice honest and terse, never hype a small sample. Validate JSON parses, commit, push.
